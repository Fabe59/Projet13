...
from accomodation.forms import CommentForm
...
class Accomodation_Views_Test(TestCase):
    ...
    def test_search_view_valid(self):
        self.client.login(username=self.username, password=self.password)
        response = self.client.get('/accomodation/search/?search=%s' %('Pluneret'))
    ...
    def test_search_view_noresult(self):
        self.client.login(username=self.username, password=self.password)
        response = self.client.get('/accomodation/search/?search=%s' %('Paris'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_details_view(self):
        ...
        response = self.client.get('/accomodation/details/%d' %(test.auto_increment_id))
        self.assertEqual(response.status_code, 200)
    ...
    def test_validation_waiting_view(self):
        ...
        waiting = AddAccomodation.objects.filter(addAccomodation_statut="Non_lu")
        response = self.client.get(reverse('accomodation:validation_waiting'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'accomodation/validation_waiting.html')
        self.assertQuerysetEqual(
    ...
    def test_validation_checked_view(self):
        self.client.login(username=self.admin_username, password=self.admin_password)
        checked = AddAccomodation.objects.get(addAccomodation_name="Maison")
        checked.lat = "12.123456"
        checked.lon = "42.123456"
        checked.save()
        verify = Accomodation.objects.filter(lat=checked.lat).filter(lon=checked.lon)
        verify2 = Accomodation.objects.filter(lat=self.accomodation.lat).filter(lon=self.accomodation.lon)
        self.assertFalse(verify)
        self.assertTrue(verify2)
        response = self.client.get(reverse('accomodation:validation_checked'), follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'accomodation/validation_waiting.html')

    def test_validation_refused_view(self):
        self.client.login(username=self.admin_username, password=self.admin_password)
        accomodation_refused = AddAccomodation.objects.get(addAccomodation_auto_increment_id=self.accomodation_waiting.addAccomodation_auto_increment_id)
        #response = self.client.post(reverse('accomodation:validation_refused'))
        #self.assertEqual(response.status_code, 302)
